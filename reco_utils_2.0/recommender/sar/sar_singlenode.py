...
class SARSingleNode:
    ...
    def __init__(
        ...
        if similarity_type not in [COOCCUR, JACCARD, LIFT, "ingredients"]:
            raise ValueError(
    ...
    def fit(self, df, ratings, col_itemid, col_ingredients):
        ...
        elif self.similarity_type is "ingredients":
            self.item_similarity = self.jaccard_matrix(ratings, col_itemid, col_ingredients) 
    ...
    def jaccard_matrix(self, df, col_id, col_sim):
        ...
        for i in range(0, num_items):
            for j in range(0, num_items): # case j > i redundant, could stop earlier
                ...
                sim_matrix[index_i,index_j] =  jaccard_anton(df.loc[i][col_sim], df.loc[j][col_sim])
        return sim_matrix

def jaccard_anton(a, b):
