...
def write_reviews(n_users, n_recipes_per_cuisine, review_ratio, review_ratio_fav):
    ...
    path_users_df = DATA_DIR + 'synthetic/users/nondet_{}_users_{}_recipepercuisine_{}_ratio_{}_ratiofav.csv'.format(n_users, n_recipes_per_cuisine, review_ratio, review_ratio_fav)
    path_reviews_df = DATA_DIR + 'synthetic/reviews/nondet_{}_users_{}_recipepercuisine_{}_ratio_{}_ratiofav.csv'.format(n_users, n_recipes_per_cuisine, review_ratio, review_ratio_fav)
...
def createUserReviews(n_users, recipe_df, review_ratio, review_ratio_fav):
    ...
    def get_rating(x):
        if x["cuisine"] == x["fav_cuisine"]:
            ...
            return randint(1,5)#cuisine_similarities[x["cuisine"]][x["fav_cuisine"]]*4 + 1  #+ 0.3 * randint(1,5)
...
def train_and_score(n_users, n_recipes_per_cuisine, review_ratio, review_ratio_fav, top_k, weight_ratings):
    ...
    review_fn = data_path + '/reviews/nondet_{}_users_{}_recipepercuisine_{}_ratio_{}_ratiofav.csv'.format(n_users, n_recipes_per_cuisine, review_ratio, review_ratio_fav)
